...
void SocialBehavior::update(Agent* agent, float deltaTime)
{
	...
	int savedDecay = agent->getSocialDecay();
	...
	{
		int socialRefill = agent->getSocial();
		//Temporarilly set the decay to 0
		agent->setSocialDecay(0);
		//While the current need is below the maximum
		while (agent->getSocial() < agent->m_startSocial)
		{
			socialRefill++;
			agent->setSocial(socialRefill);
		}
		agent->setSocialDecay(savedDecay);
	}
}
