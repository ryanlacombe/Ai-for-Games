...
void WanderBehavior::update(Agent* agent, float deltaTime)
{
	...
	//{
		...
		//agent->m_startHunger->currentStat = (agent->m_startHunger->currentStat - agent->getHungerDecay() * deltaTime);
		//agent->m_startThirst->currentStat = (agent->m_startThirst->currentStat - agent->getThirstDecay() * deltaTime);
